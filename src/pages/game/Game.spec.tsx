...
const renderGame: RenderFn = ({
  boardConfig,
  playersConfig,
}) => render(
  <Game boardConfig={boardConfig} playersConfig={playersConfig} />
);
...
const customBoardConfig = {
  ...
  layout: [
    { ship: 'destroyer', positions: [[0, 0], [0, 1]] },
  ],
};
...
describe('Game', () => {
  it('should not load board when config file is invalid', async () => {
    renderGame({
      boardConfig: { ...customBoardConfig, shipTypes: { destroyer: { size: 1, count: 1 } } },
  ...
  it('should have a winning state', async () => {
    renderGame({ boardConfig: customBoardConfig, playersConfig: customPlayersConfig });

    const boardSquares: Array<Promise<HTMLElement>> = [];
    [[0, 0], [0, 1]].forEach(coordinates => {
      boardSquares.push(screen.findByTestId(`board-square-${coordinates.join(',')}`));
    });
